...
```{r}
cluster_list_con <- clusters_create(input_list=setup_list1,n_clusters=100,
                                    benchmark_mean=3.1, benchmark_stdev=1.0, int_mean=0.0, int_stdev=0.0)
```
...
```{r}
cluster_list_int <- clusters_create(input_list=setup_list1,n_clusters=100,
                                    benchmark_mean=3.1, benchmark_stdev=1.0,int_mean=0.1, int_stdev=0.0)
```

As with the main population data, the cluster data can be saved to files for future use. Here the control and intervention cluster data sets are saved to two existing files inside the inst/extdata sub-folder.
