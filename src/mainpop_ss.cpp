...
Rcpp::List rcpp_mainpop_ss(List params, List trial_params)
{
        ...
        string file_start = rcpp_to_string(trial_params["file_start"]);
        ...
        {
        starting_data = fopen(file_start.c_str(), "r");
        for(pos=0;pos<n_cats;pos++){fscanf(starting_data,"%lf",&S[pos]);}
        for(pos=0;pos<n_cats;pos++){fscanf(starting_data,"%lf",&T[pos]);}
        for(pos=0;pos<n_cats;pos++){fscanf(starting_data,"%lf",&D[pos]);}
        for(pos=0;pos<n_cats;pos++){fscanf(starting_data,"%lf",&A[pos]);}
        for(pos=0;pos<n_cats;pos++){fscanf(starting_data,"%lf",&U[pos]);}
        for(pos=0;pos<n_cats;pos++){fscanf(starting_data,"%lf",&P[pos]);}
        for(pos=0;pos<n_cats;pos++){fscanf(starting_data,"%lf",&ICA[pos]);}
        for(pos=0;pos<n_cats;pos++){fscanf(starting_data,"%lf",&ICM[pos]);}
        for(pos=0;pos<n_cats;pos++){fscanf(starting_data,"%lf",&IB[pos]);}
        for(pos=0;pos<n_cats;pos++){fscanf(starting_data,"%lf",&ID[pos]);}
        for(pos=0;pos<n_cats;pos++){Y[pos] = S[pos] - A[pos] - U[pos];}
        fclose(starting_data);
                ...
                for (i = 0; i < na; i++) //Run through age categories
                {
                        for (j = 0; j < num_het; j++) //Run through heterogeneity categories
                        {
                                ...
                                S[pos] = den_het[pos]*0.99;
                                T[pos] = 0.0;
                                D[pos] = den_het[pos]*0.01;
                                A[pos] = 0.0;
                                U[pos] = 0.0;
                                P[pos] = 0.0;
                ...
                for (i = 0; i < na; i++) //Run through age categories
                {
                        for (j = 0; j < num_het; j++) //Run through heterogeneity categories
                        {
                                ...
                                /*if(isnan(Y[pos])!=0)
                                {
                                    Rcout << "\nY=nan at (" << i << "," << j << ")\tden_het=" << den_het[pos] << "\tdelta=" << delta[i];
                                    Rcout << "\nT = " << T[pos2] << "\tD = " << D[pos2] << "\tP = " << P[pos2] << "\nbetaT = " << betaT[pos] << "\tbetaD = " << betaD[pos] << "\tbetaP = " << betaP[pos];
                                    Rcout << "\naT = " << aT[pos] << "\naD = " << aD[pos] << "\naP = " << aP[pos];
                                    goto finish;    
                                    }*/
                                }
                                ...
                                IB[pos] = ((i == 0 ? 0.0 : IB[pos2]) + (rate_ibaq[pos] * x_I[i])) / (1.0 + (x_I[i] / db));
                                ID[pos] = ((i == 0 ? 0.0 : ID[pos2]) + (rate_detaq[pos] * x_I[i])) / (1.0 + (x_I[i] / dd));
                                /*if (isnan(S[pos]+T[pos]+D[pos]+A[pos]+U[pos]+P[pos]+ICM[pos]+ICA[pos]+IB[pos]+ID[pos]) != 0 )
                                {
                                    Rcout << "\nS(" << i << "," << j << ") = " << S[pos]<< "\tT = " << T[pos] << "\tD = " << D[pos] << "\tA = " << A[pos] << "\tU = " << U[pos] << "\tP = " << P[pos];
                                    Rcout <<"\nICA = " << ICA[pos] << "\tICM = " << ICM[pos] << "\tIB = " << IB[pos] << "\tID = " << ID[pos];
                                    goto finish;
                                }*/
                                pos++;
                        }
